...
def run_sequence(args, scene, setup, hash_size):
    ...
    # --- Updated to include setup in experiment name ---
    exp_name = f"{setup}_T{hash_size}"
    ...
    # Set Flags
    setup_params = SETUP_DEFINITIONS[setup]
    ...
    # 2. Run CoSLAM
    print(f"  [1/3] Running CoSLAM (Scene={scene}, Setup={setup}, Hash={hash_size})...")
...
def main():
    ...
    parser.add_argument("--setups", nargs="+", default=DEFAULT_SETUPS, choices=["A", "B", "C", "D"], help="List of setups to run.")
    ...
    parser.add_argument("--output-csv", type=Path, default="output/replica_results_setups.csv")
    ...
    if not args.output_csv.exists():
        ...
        with open(args.output_csv, "w") as f:
            f.write("Scene,Setup,HashSize,Accuracy,Completion,CompletionRatio,DepthL1\n")

    # Main Loop
    for scene, setup, size in itertools.product(args.scenes, args.setups, args.sizes):
        metrics = run_sequence(args, scene, setup, size)
        
        if metrics:
            row = f"{scene},{setup},{size},{metrics['Accuracy']},{metrics['Completion']},{metrics['Completion_Ratio']},{metrics['Depth_L1']}"
            print(f"  > Done: {row}")
            with open(args.output_csv, "a") as f:
                f.write(row + "\n")
        else:
            print(f"  > Failed: {scene} Setup {setup} T{size}")
