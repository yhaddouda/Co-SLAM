...
SETUP_DEFINITIONS = {
    "A": {"hash": "CoherentPrime", "morton2D": False},
    "D": {"hash": "Morton",        "morton2D": False},
    "C": {"hash": "CoherentPrime", "morton2D": True},
    "B": {"hash": "Morton",        "morton2D": True},
}

# Scenes specific to TUM (used if auto-scan fails or for default reference)
DEFAULT_TUM_SCENES = ["office0", "office1", "office2", "office3","office4", "room0", "room1", "room2"]
...
def override_config(base_cfg_path: Path, scene: str, setup_key: str, 
    ...
    cfg["grid"]["morton2D"] = setup_params["morton2D"]
...
def main():
    parser = argparse.ArgumentParser(description="Batch Runner for Co-SLAM on Replica Dataset")
    ...
    # --- Execution & Logging ---
    parser.add_argument("--log-file", default="output/replica_execution_times_2D.csv", type=Path,
    ...
    if not args.configs_root.exists():
        ...
        print("Please ensure you have created the 'configs/Replica' directory.")
        sys.exit(1)
