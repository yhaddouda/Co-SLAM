...
def build_exp_name(config_name: str, hash_size: int, run_idx: int | None = None) -> str:
...
def apply_config_mode(cfg: dict, config_name: str) -> None:
    ...
    # Keep this ablation isolated to the two requested flags.
...
def run_sequence(args, scene, config_name, hash_size, run_idx=None):
    ...
    # 2. Run CoSLAM
    if run_idx is None:
        print(f"  [1/3] Running CoSLAM (Scene={scene}, Config={config_name}, Hash={hash_size})...")
    else:
        print(f"  [1/3] Running CoSLAM (Run={run_idx}, Scene={scene}, Config={config_name}, Hash={hash_size})...")
...
def main():
    ...
    parser.add_argument(
        ...
        help="Ablation config mode(s): A, B, C, D.",
    )
    parser.add_argument(
        "--sampling",
        nargs="+",
    ...
    parser.add_argument("--output-csv", type=Path, default="output/replica_tracking_ba_ablation_results.csv")
    parser.add_argument(
        "--output-prefix",
    ...
    csv_header = (
        "Run,Scene,Config,HashSize,Accuracy,Completion,CompletionRatio,DepthL1"
        if use_run_column
        else "Scene,Config,HashSize,Accuracy,Completion,CompletionRatio,DepthL1"
    )
    ...
    # Main Loop
    total_runs = len(args.scenes) * len(args.configs) * len(args.sizes) * args.runs
    ...
    for run_idx in range(1, args.runs + 1):
        ...
        for scene, config_name, size in itertools.product(args.scenes, args.configs, args.sizes):
            effective_run_idx = run_idx if use_run_column else None
            metrics = run_sequence(args, scene, config_name, size, run_idx=effective_run_idx)

            if metrics:
                if use_run_column:
                    row = (
                        f"{run_idx},{scene},{config_name},{size},{metrics['Accuracy']},"
                        f"{metrics['Completion']},{metrics['Completion_Ratio']},{metrics['Depth_L1']}"
                    )
                else:
                    row = (
                        f"{scene},{config_name},{size},{metrics['Accuracy']},"
                        f"{metrics['Completion']},{metrics['Completion_Ratio']},{metrics['Depth_L1']}"
                    )
                print(f"  > Done: {row}")
                ...
                print(f"  > Failed: {prefix}{scene} {config_name} T{size}")
